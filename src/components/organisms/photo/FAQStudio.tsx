          ...
          <div className="md:col-span-6">
            ...
            >
              {data.items.map((item, i) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const it: any = item;
                const q = locale === "es" && it.q_es ? it.q_es : item.q;
                const a = locale === "es" && it.a_es ? it.a_es : item.a;
                return (
                  <Accordion.Item
                    key={i}
                    value={`faq-${i}`}
                    className="rounded-2xl bg-gray-2 overflow-hidden"
                  >
                    <Accordion.Header asChild>
                      <h3 className="m-0">
                        <Accordion.Trigger
                          onClick={() => onOpen(i)}
                          className="group w-full flex items-center justify-between gap-6 px-6 md:px-8 py-5 md:py-7 text-left cursor-pointer"
                        >
                          <span
